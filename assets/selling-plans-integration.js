/** Note: to make this work with radio buttons, we need to swap out the event listener on lines 12/13
 *  we then need to swap out the commented lines for the following functions:
...
class SellingPlansWidget {
  constructor(sellingPlansWidgetContainer) {
    ...
    this.listenToSellingPlanFormSelectChange();
    this.updatePrice();
  }
  ...
  get shopifySection() {
    return document.querySelector(`#product-${this.sectionId}`);
  }
  ...
  */
  ...
  get visibleSellingPlanForm() {
    return this.shopifySection.querySelector(`section[data-variant-id^="${this.variantIdInput.value}"]`);
  }
  ...
  */
  ...
  get selectedPurchaseOptionPrice() {
    ...
    return this.selectedPurchaseOption[this.selectedPurchaseOption.selectedIndex].dataset.variantPrice;
  }

  get selectedPurchaseOptionComparedAtPrice() {
    // return this.selectedPurchaseOption.dataset.variantCompareAtPrice;
    return this.selectedPurchaseOption[this.selectedPurchaseOption.selectedIndex].dataset.variantCompareAtPrice;
  }
  ...
  */
  ...
  get sellingPlanInputValue() {
    ...
    return this.selectedPurchaseOption?.value ?? null;
  }

  get selectedPurchaseOption() {
    // return this.visibleSellingPlanForm?.querySelector('input[type="radio"]:checked');
    return this.visibleSellingPlanForm?.querySelector('select[name="selling-plan"]');
  }
  ...
  handleSelectChange(selectedPurchaseOption) {
    this.selectedPurchaseOption = selectedPurchaseOption;
    this.updateSellingPlanInputsValues();
    this.updatePrice();
  }
  ...
  listenToSellingPlanFormSelectChange() {
    this.visibleSellingPlanForm?.querySelectorAll('select').forEach((select) => {
...
// Added a mutation observer to listen for modal windows being populated and add event listeners to the selling plan widget
const targetNode = document.querySelectorAll('.quick-add-modal__content-info');
...
const callback = (mutationList, observer) => {
  for (const mutation of mutationList) {
    if (mutation.type === 'childList') {
      const modalPlanSelector = mutation.target.querySelector('.selling_plan_app_container');
      new SellingPlansWidget(modalPlanSelector);
